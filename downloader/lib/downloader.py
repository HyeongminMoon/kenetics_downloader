...
def download_video(video_id, download_path, video_format="mp4", log_file=None):
    ...
    # print("Downloading = {}".format(video_id))
        # print("enter download_video pid:", os.getpid())
        ...
        # print("after download_video pid:", os.getpid())
        ...
        # print("after check download_video pid:", os.getpid())
        success = True
    ...
    if log_file is not None:
        stderr.close()
    # print("return download_video pid:", os.getpid())
...
def cut_video(raw_video_path, slice_path, start, end):
    ...
    return_code = subprocess.call(["ffmpeg", "-loglevel", "quiet", "-i", raw_video_path, "-strict", "-2", "-ss", str(start), "-to", str(end), slice_path])
...
def download_class_parallel(class_name, videos_dict, directory, videos_queue):
    ...
    ####
    # timeout = 120
    print("download_class_parallel, pid:", os.getpid())
    ...
    for key in videos_dict.keys():
        ...
        ####
        print(f"put (key, class_dir, start, end): {key}, {class_dir}, {start}, {end} pid:", os.getpid())
#         start = time.time()
#         while time.time() - start <= timeout:
#             time.sleep(.1)
            
#             print("timed out, killing worker pid:{}".format(os.getpid())
#                     r_worker.terminate()
#                     r_worker.join()
#                     self.workers.remove(r_worker)
