...
import lib.config_normal as config
...
def download_classes(classes, num_workers, failed_save_file, compress, verbose, skip, log_file):
    ...
    for list_path, save_root in tqdm(zip([config.TRAIN_METADATA_PATH], [config.TRAIN_ROOT])):
        with open(list_path) as file:
            data = json.load(file)

        pool = parallel.Pool(classes, data, save_root, num_workers, failed_save_file, compress, verbose, skip, log_file=log_file)
        pool.start_workers()
        pool.feed_videos()
        pool.stop_workers()
...
def download_missing(data, save_root, num_workers, failed_save_file, compress, verbose, skip, log_file, stats_file=None):
    ...
    pool = parallel.Pool(None, data, save_root, num_workers, failed_save_file, compress, verbose, skip, log_file=log_file, stats_file=stats_file)
    pool.start_workers()
    pool.feed_videos()
    pool.stop_workers()
...
def download_classes_from_file(classes_file, num_workers, failed_save_file, compress, verbose, skip, log_file, stats_file=None):
    ...
    for list_path, save_root in tqdm(zip([config.TRAIN_METADATA_PATH, config.VAL_METADATA_PATH], [config.TRAIN_ROOT, config.VALID_ROOT])):
        with open(list_path) as file:
            data = json.load(file)
...
def main(args):
    ...
    download_classes(args.classes, args.num_workers, args.failed_log, args.compress, args.verbose, args.skip, args.log_file)
